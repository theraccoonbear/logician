...
describe('drawCards', () => {
  it('draws from the deck when enough cards remain', () => {
    const deck = ['a', 'b', 'c', 'd']
    const { drawn, remaining, remainingDiscard } = drawCards(deck, [], 2)
  ...
  it('reshuffles the discard into the deck when the deck runs out', () => {
    const deck = ['a']
    const discard = ['b', 'c', 'd']
    const { drawn, remaining, remainingDiscard } = drawCards(deck, discard, 3)
